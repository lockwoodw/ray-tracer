...
Matrix CameraTransform(double scale) {
    Point from { 10 * scale, 2 * scale, 7 * scale }, to { 0, 0.5 * scale, -7 * scale };
    Vector up { 0, 1, 0 };
...
int main(int argc, char** argv) {
    ...
    Light light { Point { 0, 0, 1000 * scale }, {0.3, 0.3, 0.3} };
    world.Add(&light);
    ...
    for (size_t i = 0; i < n_sheets; i++) {
        ...
        GradientPattern* ptn = new GradientPattern(colours[i % n_sheets], Colour::kBlack);
